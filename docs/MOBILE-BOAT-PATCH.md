...
> **Superseded (2026-09-15).** The full mobile journey from `codex/mobile-journey` was merged into production. Phones now use the same scroll-driven journey as desktop, so the stacked layout and this interim boat motion are no longer mounted (`JourneyStacked.tsx` is kept unmounted for reference). See `docs/briefs/MOBILE-JOURNEY-HANDOFF.md`.

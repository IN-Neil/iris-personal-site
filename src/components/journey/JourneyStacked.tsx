...
function Frame({ progress, tall = false }: { progress: number; tall?: boolean }) {
  return (
    <div
      className={`relative w-full overflow-hidden border-y border-ink/20 ${tall ? "aspect-[3/4]" : "aspect-[4/3]"}`}
    >
    ...
    <div className="bg-ivory">
      ...
      {chapters.map((chapter) => (
        <article key={chapter.id} className="mt-10">
          ...
          <div className="px-5 py-8">
            <ChapterPanel chapter={chapter} className="static-paper" />
          </div>
        </article>
      ))}
      ...
      <article className="mt-6">
        ...
        <div className="px-5 pb-12 pt-8">
          <EndingPanel className="static-paper" />
        </div>
      </article>
    </div>
  );
